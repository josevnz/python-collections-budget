...
def test_task2_module2():
    ...
    assert overages_assign_found, 'Did you initialize `sum_overages` to 0 and `overages` to an empty list in ' \
                                  '`__init__`()? '
...
def test_task4_module2():
    ...
    try:
        ...
        # TODO Why this is hidden?
        # print('append e = ' + str(e))
        pass

    assert append_def_found, 'Did you define the method `def append(self, item)`?'
    assert self_found and sum_expenses_found and op_add_found and lt_found and self_budget_found, 'Add an `if` ' \
                                                                                                  'statement that ' \
                                                                                                  'checks if ' \
                                                                                                  '`self.sum_expenses' \
                                                                                                  '+item < ' \
...
def test_task11_module2():
            ...
            if (isinstance(x, ast.FunctionDef) and
                ...
                for y in x.body:
                    if (isinstance(y, ast.Expr) and
                        ...
                        # This code doesn't work with an f-string
    ...
    assert print_call_found, 'Did you print `\'The count of all expenses: \'` concatenated with `str(len(' \
                             'myBudgetList))`? '
