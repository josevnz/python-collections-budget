...
def test_task4_module3():
                    ...
                    for y in x.body:
                        if (isinstance(y, ast.FunctionDef) and
                            ...
                            if isinstance(y.body[0], ast.Try):
                                ...
                                """
                                NOTE:
                                Original check had bugs (fixed version below): 
                                - If the user stops to declare 'stop' it also fails the next check.
                                - Also, why check on stop when the variable will not be used inside or outside 
                                  the exception block?
                                """
                                if isinstance(y.body[0].handlers[0], ast.ExceptHandler) \
                                        and y.body[0].handlers[0].type.id == 'StopIteration':
                                    if y.body[0].handlers[0].name == 'stop':
    ...
    # Useless check: Why declare stop as a variable if is not going to be used?
...
def test_task5_module3():
    ...
    assert for_mybudgetlist_found, 'Did you create a for loop that iterates `myBudgetList`, with `entry` as the name ' \
                                   'of the iterator? '
                ...
                for y in x.body:
                    if (isinstance(y, ast.Assign) and
                        ...
                        if (y.value.elts[0].value.id == 'myBudgetList' and y.value.elts[0].attr == 'sum_expenses' and
                                y.value.elts[1].value.id == 'myBudgetList' and y.value.elts[
                                    1].attr == 'sum_overages' and
