...
def test_task4_module4():
    ...
    assert unnecessary_expenses_found, 'Did you create the variable `unnecessary_expenses`? It should be initialized ' \
                                       'to `set(self.list)` minus `necessary_expenses` minus `food_expenses`. '
...
def test_task6_module4():
    ...
    message = 'Did you call `categorize_set_comprehension()` on `expenses` and assign the result to a variabled named ' \
              '`divided_set_comp`? '
...
def test_task9_module4():
    ...
    for_str = 'a:b:zip:divided_for_loop:divided_set_comp:a:issubset:b:b:issubset:a:print:Sets are NOT equal by subset ' \
              'test '
    message = 'Did you create an `for` loop that uses `a,b` as iterator variables to iterate `zip(divided_for_loop, ' \
              'divided_set_comp)`? '
    """
